...
RegisterNetEvent("gcphone:callData:cb")
AddEventHandler(
  "gcphone:callData:cb",
  function(data)
    local gotPhone = false
    for k, v in pairs(data.Inventaire) do
      if v.id == 206 then
        gotPhone = true
      end
    end
    if gotPhone == true then
      TooglePhone()
    else
      ShowNoPhoneWarning()
    end
...
RegisterNetEvent("gcphone:notification")
AddEventHandler(
  "gcphone:notification",
