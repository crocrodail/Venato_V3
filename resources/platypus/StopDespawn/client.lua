...
AddEventHandler('sd:checkVehs', function(table)
  ...
  for i=1,#table,1 do
    ...
    print(CarOnPoint)
    print(plate)   
    print(table[i].id..' - Network Id')   
    print(platypus.dump(entity))   
    print(entityPlate)   
    print('*****')     
		-- if CarOnPoint == nil or plate ~= table[i].plate then
    --   local newId = spawnVehicle(table[i].model, table[i].position.x, table[i].position.y, table[i].position.z, table[i].heading, table[i].plate, table[i].health, table[i].customs)
