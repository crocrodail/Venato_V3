  ...
  while true do
    ...
    if IsEntityDead(playerPed) then
      ...
      if not dead or (assommePlayer and causeOfDeath ~= old_cause) then      
          ...
          platypus.playAnim({lib = "mini@cpr@char_b@cpr_def", anim = "cpr_pumpchest_idle", useLib = true, flag = 1})
    ...
    if dead then
      ...
      --platypus.playAnim({lib = "mini@cpr@char_b@cpr_def", anim = "cpr_pumpchest_idle", useLib = true, flag = 2})
    end
  end
end)
...
AddEventHandler("Death:ComaOrNot:cb", function(boolean)
  Citizen.CreateThread(function()    
    ...
    platypus.ScaleForm("MP_BIG_MESSAGE_FREEMODE")
...
AddEventHandler("vnt:heal:cb", function()
  SetEntityHealth(platypus.GetPlayerPed(), 200.0)
  TriggerServerEvent("Death:health", false)
end)
...
function Reanim(char, coord, heading)
  Citizen.CreateThread(function()
    ...
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_def", anim = "cpr_intro", useLib = true})
    Citizen.Wait(15000)
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_str", anim = "cpr_pumpchest", useLib = true, flag = 1})
    Citizen.Wait(5000)
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_str", anim = "cpr_cpr_to_kol", useLib = true})
    Citizen.Wait(1500)
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_str", anim = "cpr_kol", useLib = true})
    Citizen.Wait(5000)
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_str", anim = "cpr_kol_to_cpr", useLib = true})
    Citizen.Wait(1500)
    platypus.playAnim({lib = "mini@cpr@char_"..char.."@cpr_str", anim = "cpr_pumpchest", useLib = true, flag = 1})
    Citizen.Wait(5000)
