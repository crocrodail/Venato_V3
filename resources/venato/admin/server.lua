...
AddEventHandler("Admin:ActionOnPlayer", function(action, target, msg)
	...
	if action == "kick" then
		...
		MySQL.Async.execute("INSERT INTO bans (`banned`, `banner`, `reason`, `ip`) VALUES (@banned, @banner, @reason, @ip)", {["@banned"] = DataPlayers[target].SteamId, ["@banner"] = DataPlayers[tonumber(source)].SteamId, ["@reason"] = msg, ["@ip"] = DataPlayers[target].Ip}, function()
