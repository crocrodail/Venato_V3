...
AddEventHandler("Life:UpdateDB", function(needs)  
    local source = source
    UpdateDb(source, needs)
end)

RegisterNetEvent("Life:Eat")
AddEventHandler("Life:Eat", function(qte)  
...
AddEventHandler("Life:Drink", function(qte)  
...
function UpdateDb(source, needs)
    MySQL.Sync.execute("UPDATE users SET food = @food, water = @water, sool = @alcool WHERE identifier = @identifier", {
        ['@food'] = needs.food,
        ['@water'] = needs.water,
        ['@alcool'] = needs.alcool,
