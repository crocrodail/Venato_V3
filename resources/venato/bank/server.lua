...
function CheckPlafondRetrait(source, amount)
  local result = MySQL.Sync.fetchAll("SELECT SUM(montant) + @amount as montant FROM bank_transactions WHERE identifier = @SteamId and isDepot = 0 AND date BETWEEN DATE_ADD(CURRENT_DATE, INTERVAL -7 DAY) AND DATE_ADD(CURRENT_DATE, INTERVAL 7 DAY)",{["@SteamId"] = DataPlayers[source].SteamId, ["@amount"] = amount})
