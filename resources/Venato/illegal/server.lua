...
AddEventHandler('illegal:transform', function(drugId)
    local source = source
    local drugItem = drugs[drugId]
    if(platypus.CheckItem(drugItem.farm.item_id, source) < drugItem.transform.transform_qte) then
...
AddEventHandler('illegal:sell', function(drugId)
    local source = source
    local drugItem = drugs[drugId]
    if(platypus.CheckItem(drugItem.sell.item_id, source) < drugItem.sell.sell_qte) then
