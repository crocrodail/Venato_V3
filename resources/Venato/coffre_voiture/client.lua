...
function OpenMenuCv()
  ...
  TriggerEvent(
    "Menu:Init",
    color .. "" .. VehicleData.nbItems .. "</span> / " .. VehicleData.itemcapacite,
    "Coffre",
    "#2E7D3299",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYDjg60jfde2J-ii7xbyTuU8K8FC1vjK2ebUNTrchvqbTIGI68Xg"
  )
  Menu.addItemButton(
    "<span class='red--text'>Fermer le coffre</span>",
  ...
  for k, v in pairs(VehicleData.inventaire) do
    Menu.addItemButton(
      "" .. v.libelle .. " : </span><span class='red--text'>" .. v.quantity .. "</span>",
      v.picture,
      "OptionItemsCv",
      k
    )
...
function DropItemCv()
  ...
  for k, v in pairs(DataUser.Inventaire) do
    if v.quantity ~= 0 then
      Menu.addItemButton(
        v.libelle .. " : <span class='red--text'>" .. v.quantity .. "</span>",
        v.picture,
        "ConfDropItemCv",
        v.id
      )
...
function DropWeaponCv()
  ...
  for k, v in pairs(DataUser.Weapon) do
    Menu.addButton(
      v.libelle .. " avec <span class='red--text'>" .. v.ammo .. "</span> balles",
      "DropConfirmWeaponCv",
      k
    )
  end
