...
AddEventHandler(
  "VehicleCoffre:CallData",
  function(plate, class)
    local key = ""
    local source = source
    local Cof = {}
    local CofWp = {}
    ...
    if key == "" then
      MySQL.Async.fetchAll(
        "SELECT * FROM coffres_voiture_contenu JOIN items ON coffres_voiture_contenu.ItemId = items.id WHERE CoffreId = @id",
        {["@id"] = plate},
        function(result)
          ...
          MySQL.Async.fetchAll(
            "SELECT CW.Id, CW.CoffreId, W.libelle, CW.balles, W.poids, W.id as weaponId, W.weapon_id FROM coffres_voiture_weapons CW JOIN weapon W ON CW.Weapon = W.id WHERE CW.CoffreId = @id",
            {["@id"] = plate},
            function(resultWp)
              if resultWp ~= nil then
                for k, v in pairs(resultWp) do
                  CofWp = {
                    ["weaponId"] = v.weaponId,
                    ["weaponModel"] = v.weapon_id,
                    ["libelle"] = v.libelle,
                    ["balles"] = math.floor(v.balles),
                    ["poids"] = v.poids
                  }
                  weapon[v.Id] = CofWp
...
AddEventHandler(
  "VehicleCoffre:DropItem",
  function(qty, plate, index)
    local source = source
    local qty = tonumber(qty)
    if
      qty <= DataPlayers[tonumber(source)].Inventaire[index].quantity and
        DataVehicle[plate].nbItems + qty <= DataVehicle[plate].itemcapacite and
        qty > 0
     then
...
AddEventHandler(
  "VehicleCoffre:TakeItems",
  function(index, qty, plate)
    local source = source
    local qty = tonumber(qty)
    if
      qty <= DataVehicle[plate].inventaire[index].quantity and
        DataPlayers[tonumber(source)].Poid + DataVehicle[plate].inventaire[index].poid * qty <=
          DataPlayers[tonumber(source)].PoidMax and
        qty > 0
     then
...
AddEventHandler(
  "VehicleCoffre:SetItems",
  function(qty, id, plate)
    local source = source
    local qty = tonumber(qty)
    if qty < 1 then
      ...
      MySQL.Async.execute(
        "DELETE FROM coffres_voiture_contenu WHERE CoffreId = @Plate AND ItemId = @ItemId",
        {["@Plate"] = plate, ["@ItemId"] = id}
      )
    else
      if DataVehicle[plate].inventaire[id] ~= nil then
        ...
        MySQL.Async.execute(
          "UPDATE coffres_voiture_contenu SET Quantity = @qty WHERE CoffreId = @Plate AND ItemId = @ItemId",
          {["@Plate"] = plate, ["@ItemId"] = id, ["@qty"] = qty}
        )
      else
        MySQL.Async.fetchAll(
          ...
          function(result)
            ...
            DataVehicle[plate].inventaire[id] = {
            ...
            MySQL.Async.execute(
              "INSERT INTO coffres_voiture_contenu (`CoffreId`, `ItemId`, `Quantity`) VALUES (@plate, @id, @qty)",
              {["@plate"] = plate, ["@id"] = id, ["@qty"] = qty}
            )
          end
        )
      end
...
AddEventHandler(
  "VehicleCoffre:TakeWpCv",
  function(index, plate)
    local source = source
    if
      DataPlayers[tonumber(source)].Poid + DataVehicle[plate].weapon[index].poids <=
        DataPlayers[tonumber(source)].PoidMax
     then
      TriggerEvent(
        "Inventory:AddWeapon",
        DataVehicle[plate].weapon[index].weaponModel,
        DataVehicle[plate].weapon[index].balles,
        DataVehicle[plate].weapon[index].poids,
        DataVehicle[plate].weapon[index].libelle,
        source
      )
...
AddEventHandler(
  "VehicleCoffre:DropWpCv",
  function(index, plate)
    local source = source
    if DataVehicle[plate].nbWeapon + 1 <= DataVehicle[plate].maxWeapon then
      MySQL.Async.execute(
        "INSERT INTO coffres_voiture_weapons (`Weapon`, `CoffreId`, `balles`) VALUES (@weapon, @plate, @balles)",
        {
          ["@weapon"] = DataPlayers[tonumber(source)].Weapon[index].idWeapon,
          ["@plate"] = plate,
          ["@balles"] = DataPlayers[tonumber(source)].Weapon[index].ammo
        },
        function()
          MySQL.Async.fetchAll(
            "SELECT CW.Id, W.libelle, W.poids, W.weapon_id  FROM coffres_voiture_weapons CW join weapon W ON CW.Weapon = W.id WHERE CW.Weapon = @weapon AND CW.CoffreId = @coffreId ORDER BY Id DESC",
            {
              ["@weapon"] = DataPlayers[tonumber(source)].Weapon[index].idWeapon,
              ["@coffreId"] = plate
            },
            function(result)
              DataVehicle[plate].weapon[result[1].Id] = {
                ["weaponId"] = DataPlayers[tonumber(source)].Weapon[index].idWeapon,
                ["weaponModel"] = DataPlayers[tonumber(source)].Weapon[index].id,
                ["libelle"] = result[1].libelle,
                ["balles"] = DataPlayers[tonumber(source)].Weapon[index].ammo or 0,
                ["poids"] = result[1].poids
              }
              TriggerEvent(
                "Inventory:RemoveWeapon",
                DataPlayers[tonumber(source)].Weapon[index].id,
                index,
                result[1].poids,
                source
              )
