...
export default {
  ...
  methods: {
    ...mapActions(["loadConfig", "rejectCall", "showPhone", "closePhone", "addNotification", "removeNotification"]),
  },
  computed: {
    ...mapGetters([
      "show",
  ...
  created() {
    this.$bus.$on("showPhone", () => {this.showPhone();});
    this.$bus.$on("hidePhone", () => {this.closePhone();});
  },
  mounted() {
    this.loadConfig();
...
beforeDestroy(){
  this.$bus.$off("showPhone", () => {this.showPhone();});
  this.$bus.$off("hidePhone", () => {this.closePhone();});
}
};
</script>
